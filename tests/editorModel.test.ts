...
describe('仕切り棒の編集', () => {
  it('仕切りの上を指すと選べる', () => {
    const m = model();
    const d = m.def.dividers[0];
    // ⚠️ 高さは**段（300/410/520）を避ける**こと。段の上ではゲートが優先されるので、
    //    仕切りの「真ん中」を突くと、そこにゲートがある配置では gate が返る
    //    （2026-07-26 既定ステージの仕切りを 180 → 90 に動かしたら、90 が y410 の
    //     ゲートの内側に入ってこのテストが落ちた）。
    expect(m.pick(d.x1, 350)).toEqual({ kind: 'divider', index: 0 });
  });
