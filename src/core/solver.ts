...
export function step(
  ...
  pool.forEachActive((b) => {
    ...
    // ⚠️ カップの底を転がっている間は**まだ盤面に出ていない**（バケツの中に居る）ので、
    //    壁にも仕切りにも当てない。当てると、左端を狙ってカップが画面から見切れている時に
    //    左壁へ押し戻され、なぞった所より右に落ちる（2026-07-25 実測: 10を狙って64に落ちた）。
    if (b.rollFrames > 0) return;
